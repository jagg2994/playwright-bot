...
| P12 | **Flujo: limpiar carrito antes de ejecutar** | Alta | Pendiente | Antes de correr los flujos, navegar al pedido y eliminar todos los productos del carrito. Sin esto, productos ya agregados en ejecuciones anteriores causan skips y el bot no puede demostrar el flujo completo. Investigar la UI de eliminación en el env de prueba (botón "Eliminar", modal de confirmación, etc.). |
| P13 | **Output de estado de flujos por ejecución** | Alta | Pendiente | Al finalizar cada ejecución, generar un archivo `flow_status_<timestamp>.json` con el resultado de cada flujo (✅ completo, ⚠️ parcial, ❌ error, causa, duración). Permite ver de un vistazo qué funcionó sin leer el log completo. |
